function [d, C] = test_adptive_DsRs_blocks(D,C,s,ds,rs)
%% 验证自适应分组算法
idx=sum(abs(C));
C=C(:,idx~=0);
...
d=1:K;
dlen=ones(1,K);
G = calculate_GDsRs_test(D,C,s,d,dlen,[],[],ds,rs);  % similarity between sparsity of blocks
while sum(sum(G>-1))
    ...
    G = calculate_GDsRs_test(D,C,s ,d, dlen, G([1:(k2-1) (k2+1):end],[1:(k2-1) (k2+1):end]), k1, ds, rs);
end